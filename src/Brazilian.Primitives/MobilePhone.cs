...
/// <summary>
/// Represents a Brazilian mobile phone number under Anatel's numbering plan.
/// </summary>
/// <remarks>
/// Validation is structural and numbering-plan based. A valid <see cref="MobilePhone"/> does not prove that the
/// number exists, is active, belongs to a specific subscriber, is reachable, or is currently associated with a
/// particular carrier. Carrier inference, portability lookup, landlines, non-geographic numbers, and service codes
/// are outside this type's scope.
/// </remarks>
public readonly record struct MobilePhone : IParsable<MobilePhone>, ISpanParsable<MobilePhone>, IFormattable
{
    private const int NationalLength = 11;
    private const int SubscriberLength = 9;
    private const int FormattedLength = 15;
    private const int E164Length = 14;
    private const int InternationalFormattedLength = 17;

    private readonly string? _value;

    private MobilePhone(string value)
    {
        _value = value;
    }

    /// <summary>
    /// Gets the canonical national representation containing DDD plus the nine-digit mobile subscriber number.
    /// </summary>
    public string Value => _value ?? throw new InvalidOperationException("A default mobile phone instance does not contain a valid value.");
    ...
    /// <summary>
    /// Gets the nine-digit mobile subscriber number, which starts with the mandatory ninth digit <c>9</c>.
    /// </summary>
    public string SubscriberNumber => Value[2..];

    /// <summary>
    /// Gets the national display representation using the <c>(00) 90000-0000</c> pattern.
    /// </summary>
    ...
    /// <inheritdoc />
    public static MobilePhone Parse(string s, IFormatProvider? provider)
    {
        if (s is null)
        {
            throw new FormatException("Mobile phone must be provided in a supported format.");
        }
    ...
    /// <inheritdoc />
    public static MobilePhone Parse(ReadOnlySpan<char> s, IFormatProvider? provider)
    {
        if (!TryNormalize(s, out string normalized))
        {
            throw new FormatException("Mobile phone must contain a valid Brazilian DDD and a nine-digit subscriber number starting with 9 in a supported format.");
        }
    ...
    /// <summary>
    /// Attempts to parse a supported Brazilian mobile phone representation.
    /// </summary>
    /// <param name="value">The phone text to parse.</param>
    /// <param name="result">When successful, contains the validated mobile phone.</param>
    /// <returns><see langword="true"/> when the input is valid; otherwise, <see langword="false"/>.</returns>
    public static bool TryParse(string? value, out MobilePhone result)
    {
        return TryParse(value, provider: null, out result);
    }

    /// <inheritdoc />
    public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider, out MobilePhone result)
    {
        if (s is null)
        {
            result = default;
    ...
    /// <inheritdoc />
    public static bool TryParse(ReadOnlySpan<char> s, IFormatProvider? provider, out MobilePhone result)
    {
        if (TryNormalize(s, out string normalized))
        {
            result = new MobilePhone(normalized);
            return true;
        }
    ...
    /// <summary>
    /// Determines whether the supplied text represents a structurally valid Brazilian mobile phone number.
    /// </summary>
    ...
    /// <summary>
    /// Returns the canonical eleven-digit national representation.
    /// </summary>
    ...
    public string ToString(string? format, IFormatProvider? formatProvider)
    {
        ...
        throw new FormatException($"Unsupported mobile phone format '{format}'. Use 'G', 'F', or 'E'.");
    }
    ...
    private static bool TryNormalize(ReadOnlySpan<char> input, out string normalized)
    {
        ...
        if (!TryExtractCanonical(input, canonical)
            ...
            || !IsMobileSubscriber(canonical[2..]))
        {
    ...
    {
        if (input[0] != '(' || input[3] != ')' || input[4] != ' ' || input[10] != '-')
        {
            return false;
        }
    ...
    private static bool IsMobileSubscriber(ReadOnlySpan<char> subscriber)
    {
        return subscriber.Length == SubscriberLength && subscriber[0] == '9';
    }
