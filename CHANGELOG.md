...
- Context-free and state-aware SQL Server persistence for `Rg` and `InscricaoEstadual`, with loss-preventing single-column converters, EF Core complex-property table splitting, stable two-letter UF codes, nullable-property coverage, and real SQL Server round-trip validation.
...
- `InscricaoEstadual` value object with optional `BrazilianState` context, conservative context-free parsing, explicit 27-UF structural strategy matrix, `ISENTO` rejection, context-preserving equality, and documented format-only validation where no UF checksum is embedded.
...
- `Rg` value object with optional `BrazilianState` context, conservative context-free parsing, explicit legacy RG strategies for all 27 UFs, strict state-aware parsing, São Paulo SSP/IIRGD check-digit validation, format-only validation where no sufficiently reliable checksum source is available, and explicit distinction from CIN.
