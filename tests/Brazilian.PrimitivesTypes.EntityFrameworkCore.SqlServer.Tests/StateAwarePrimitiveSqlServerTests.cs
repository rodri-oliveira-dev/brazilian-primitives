...
public sealed class StateAwarePrimitiveSqlServerTests
{
    ...
    [Fact]
    public void BrazilianStateConverterUsesStableUfCodesAndRejectsUnknownState()
    {
        BrazilianStateCodeValueConverter converter = new();

        Assert.Equal("SP", converter.ConvertToProvider(BrazilianState.SaoPaulo));
        Assert.Equal("RJ", converter.ConvertToProvider(BrazilianState.RioDeJaneiro));
        Assert.Equal(BrazilianState.MinasGerais, converter.ConvertFromProvider("MG"));
